...
if exist "sqlite3\sqlite3.c" goto sqlite_ok

echo [INFO] SQLite3 amalgamation not found. Downloading...
if not exist "sqlite3" mkdir sqlite3

echo [INFO] Downloading from sqlite.org ...
powershell -Command "$ProgressPreference='SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://www.sqlite.org/%SQLITE_YEAR%/sqlite-amalgamation-%SQLITE_VER%.zip' -OutFile 'sqlite3\sqlite3.zip'"
if errorlevel 1 (
    echo [ERROR] Failed to download SQLite3 amalgamation.
    echo Please download manually from https://www.sqlite.org/download.html
    echo Extract sqlite3.c and sqlite3.h into the sqlite3\ directory.
    exit /b 1
)
